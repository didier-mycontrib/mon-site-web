...
export class AuthService {
  ...
  public currentSecureMode : boolean =true; //=false; //temporairement false pour premier tests !!!!!
  public lastAuthResponse : AuthResponse = null; //last succeeding response only

  constructor(private _http : HttpClient) {
     try{
      this.getSecureMode().subscribe(()=>{console.log("secureMode="+this.currentSecureMode)})
     } catch(err){
     }
   }
