...
export abstract class GenericRestCrudService<T> {
  ...
  protected publicBaseUrl = null ; //must be set by subclass
  protected privateBaseUrl = null ; //avec ng serve --proxy-config proxy.conf.json
  ...
  public postEntityObject(obj : T):Observable<T> {
    let url : string = this.privateBaseUrl;
  ...
  public getEntityObjectById(id:any,requiredRole:string=null):Observable<T>{
    let  url : string ;
  ...
  public getAllObjects(requiredRole:string=null):Observable<T[]>{
      let  url : string ;
      if(requiredRole==null){
        url = this.publicBaseUrl ;
      }else{
        url =  this.privateBaseUrl ; //encore améliorable
      }
      return this.http.get<T[]>(url );
  }

  public findObjectsFromCriteria(criteriaEndUrl:string,requiredRole:string=null):Observable<T[]>{
    let  url : string ;
    if(requiredRole==null){
...
export abstract class GenericRestCrudServiceWithUpload<T>
   extends GenericRestCrudService<T> {
    ...
    protected uploadBaseUrl = null ; //must be set by subclass
