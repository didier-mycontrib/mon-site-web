...
var initialSecureMode = MyAppConfig.configuredSecureMode();//true(secure by default) or false (if --unsecure)
var secureMode = initialSecureMode;

export const secureModeApiRouter = Router();

//setting secureMode to true or false (tp , dev-only with --unsecure)
// GET http://localhost:8282/auth-api/dev-only/set-secure-mode/true or false renvoyant { secureMode : true_or_false}
if(initialSecureMode==false)
