...
import { isPlainTextPwdMatchDatabasePwd , cryptPwdIfNecessary } from '../util/bcrypt-util';
...
.post(asyncToResp(async function(req :Request, res :Response , next: NextFunction ) {
 ...
 if(login==null){
    ...
    const pwdMatch : boolean = await 
       isPlainTextPwdMatchDatabasePwd(authReq.password,
                                     login.password);
    if(pwdMatch){
...
.post(asyncToResp(async function(req :Request, res :Response , next: NextFunction ) {
    ...
    login.password= await cryptPwdIfNecessary(login.password);
