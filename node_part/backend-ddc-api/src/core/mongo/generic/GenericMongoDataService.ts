...
export class GenericMongoDataService<T,ID> implements BasicCrudService<T,ID>{
    ...
    private dB() : Promise<Db>{
        ...
        return new Promise((resolve,reject) => {
            ...
            //console.log("in db() " +  this.connectionName +" : initialized: " + myMongoConnection.isInitialized());
    ...
    private buildQueryById(id: ID) : object{
        ...
        if(this.idHelper.isAuto()){
            query = { '_id' :  mongoIdAsString };//this.buildMongoIdFromAnyId(mongoIdAsString)};
        }else{
    ...
    private adjustMongoId(e:T):T{
        if(this.idHelper.getIdPropName() != "_id"){
            //remplacer  .idPropName="valeurPk" par   ._id="valeurPk" :
    ...
    private adjustMongoIdOrMongoObjectId(e:T):T{
        let ee : T =  this.adjustMongoId(e);
        if(this.idHelper.isAuto()){
            let pkValue = Reflect.get(ee as any as object ,"_id");
            if(pkValue==null){
               let newPkValue=(new ObjectID()).toHexString();
               Reflect.set(ee as any as object,"_id",newPkValue);
            }
    ...
    public insert(dataObj: T): Promise<T> {
        return new Promise((resolve,reject) => {
            this.dB().then ( (db)=> { db.collection(this.collectionName)
                   .insertOne(this.adjustMongoIdOrMongoObjectId(dataObj),(err, result) =>{
                        if(err!=null) {
